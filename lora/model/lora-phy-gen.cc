...
TypeId LoraPhyCalcSinrDefault::GetTypeId(void) {
...
double LoraPhyCalcSinrDefault::CalcSinrDb(
...
TypeId LoraPhyCalcSinrFhFsk::GetTypeId(void) {
  static TypeId tid =
      TypeId("ns3::LoraPhyCalcSinrFhFsk")
          .SetParent<Object>()
          .SetGroupName("Lora")
...
double LoraPhyCalcSinrFhFsk::CalcSinrDb(
    Ptr<Packet> pkt, Time arrTime, double rxPowerDb, double ambNoiseDb,
    LoraTxMode mode, LoraPdp pdp,
    const LoraTransducer::ArrivalList &arrivalList) const {
  ...
  double isiUpa =
      rxPowerDb * pdp.SumTapsFromMaxNc(Seconds(ts + clearingTime), Seconds(ts));
  LoraTransducer::ArrivalList::const_iterator it = arrivalList.begin();
  double intKp = -DbToKp(effRxPowerDb);
  for (; it != arrivalList.end(); it++) {
    LoraPdp intPdp = it->GetPdp();
    double tDelta = std::abs(arrTime.GetSeconds() + maxTapDelay -
                             it->GetArrivalTime().GetSeconds());
...
LoraPhyPerGenDefault::~LoraPhyPerGenDefault() {}
TypeId LoraPhyPerGenDefault::GetTypeId(void) {
  static TypeId tid =
      TypeId("ns3::LoraPhyPerGenDefault")
          .SetParent<Object>()
          .SetGroupName("Lora")
...
double LoraPhyPerGenDefault::CalcPer(Ptr<Packet> pkt, double sinrDb,
                                     LoraTxMode mode) {
  if (sinrDb >= m_thresh) {
    return 0;
  } else {
...
double LoraPhyPerUmodem::NChooseK(uint32_t n, uint32_t k) {
  double result;
...
double LoraPhyPerUmodem::CalcPer(Ptr<Packet> pkt, double sinr,
                                 LoraTxMode mode) {
...
LoraPhyGen::LoraPhyGen()
    : LoraPhy(), m_state(IDLE), m_channel(0), m_transducer(0), m_device(0),
      m_mac(0), m_rxGainDb(0), m_txPwrDb(0), m_rxThreshDb(0), m_ccaThreshDb(0),
      m_pktRx(0), m_cleared(false), m_disabled(false), m_useRandomNoise(false) {
...
void LoraPhyGen::DoDispose() {
  Clear();
  m_energyCallback.Nullify();
  LoraPhy::DoDispose();
}

LoraModesList LoraPhyGen::GetDefaultModes(void) {
  LoraModesList l;
  l.AppendMode(LoraTxModeFactory::CreateMode(LoraTxMode::FSK, 80, 80, 22000,
                                             4000, 13, "FSK"));
  l.AppendMode(LoraTxModeFactory::CreateMode(LoraTxMode::PSK, 200, 200, 22000,
                                             4000, 4, "QPSK"));
  return l;
}
TypeId LoraPhyGen::GetTypeId(void) {

  static TypeId tid =
      TypeId("ns3::LoraPhyGen")
          .SetParent<LoraPhy>()
          .SetGroupName("Lora")
          ...
          .AddAttribute(
              ...
              DoubleValue(10), MakeDoubleAccessor(&LoraPhyGen::m_rxThreshDb),
              MakeDoubleChecker<double>())
          .AddAttribute("TxPower", "Transmission output power in dB.",
                        DoubleValue(190),
                        MakeDoubleAccessor(&LoraPhyGen::m_txPwrDb),
                        MakeDoubleChecker<double>())
          .AddAttribute("RxGain", "Gain added to incoming signal at receiver.",
                        DoubleValue(0),
...
void LoraPhyGen::SetEnergyModelCallback(
    DeviceEnergyModel::ChangeStateCallback cb) {
  NS_LOG_FUNCTION(this);
  m_energyCallback = cb;
}

void LoraPhyGen::UpdatePowerConsumption(const State state) {
  NS_LOG_FUNCTION(this);
...
void LoraPhyGen::EnergyDepletionHandler() {
  NS_LOG_FUNCTION(this);
...
void LoraPhyGen::SendPacket(Ptr<Packet> pkt, uint32_t modeNum) {
...
void LoraPhyGen::RegisterListener(LoraPhyListener *listener) {
  m_listeners.push_back(listener);
}

void LoraPhyGen::StartRxPacket(Ptr<Packet> pkt, double rxPowerDb,
  ...
  case RX: {
    NS_ASSERT(m_pktRx);
    double newSinrDb = CalculateSinrDb(m_pktRx, m_pktRxArrTime, m_rxRecvPwrDb,
    ...
    NS_LOG_DEBUG("PHY " << m_mac->GetAddress()
                        ...
                        << m_minRxSinrDb << "packet = " << pkt->GetUid());
  ...
  case IDLE: {
    ...
    double newsinr =
        CalculateSinrDb(pkt, Simulator::Now(), rxPowerDb, txMode, pdp);
    NS_LOG_DEBUG("PHY " << m_mac->GetAddress()
    ...
    if (newsinr > m_rxThreshDb) {
      ...
      m_pktRxPdp = pdp;
      double txdelay = pkt->GetSize() * 8.0 / txMode.GetDataRateBps();
      Simulator::Schedule(Seconds(txdelay), &LoraPhyGen::RxEndEvent, this, pkt,
                          rxPowerDb, txMode);
  ...
  auto interferenceDB = GetInterferenceDb((Ptr<Packet>)0);
  if (m_state == IDLE && interferenceDB > m_ccaThreshDb) {
    m_state = CCABUSY;
    NS_LOG_DEBUG("StartRxPacket CCABUSY: packet = " << pkt->GetUid()
      << " Interference: " << interferenceDB << "Db"
    );
    NotifyListenersCcaStart();
  }
}

void LoraPhyGen::RxEndEvent(Ptr<Packet> pkt, double rxPowerDb,
                            LoraTxMode txMode) {
  if (pkt != m_pktRx) {
    return;
  }
  ...
  auto r_val = m_pg->GetValue(0, 1);
  if (!m_useRandomNoise) {
    r_val = 1;
  }
  auto per = m_per->CalcPer(m_pktRx, m_minRxSinrDb, txMode);
  if (r_val > per) {
...
void LoraPhyGen::SetReceiveOkCallback(RxOkCallback cb) { m_recOkCb = cb; }

void LoraPhyGen::SetReceiveErrorCallback(RxErrCallback cb) { m_recErrCb = cb; }
bool LoraPhyGen::IsStateSleep(void) { return m_state == SLEEP; }
bool LoraPhyGen::IsStateIdle(void) { return m_state == IDLE; }
bool LoraPhyGen::IsStateBusy(void) { return !IsStateIdle() && !IsStateSleep(); }
bool LoraPhyGen::IsStateRx(void) { return m_state == RX; }
bool LoraPhyGen::IsStateTx(void) { return m_state == TX; }

bool LoraPhyGen::IsStateCcaBusy(void) { return m_state == CCABUSY; }

void LoraPhyGen::SetRxGainDb(double gain) { m_rxGainDb = gain; }
void LoraPhyGen::SetTxPowerDb(double txpwr) { m_txPwrDb = txpwr; }
void LoraPhyGen::SetRxThresholdDb(double thresh) { m_rxThreshDb = thresh; }
void LoraPhyGen::SetCcaThresholdDb(double thresh) { m_ccaThreshDb = thresh; }
double LoraPhyGen::GetRxGainDb(void) { return m_rxGainDb; }
double LoraPhyGen::GetTxPowerDb(void) { return m_txPwrDb; }
double LoraPhyGen::GetRxThresholdDb(void) { return m_rxThreshDb; }
double LoraPhyGen::GetCcaThresholdDb(void) { return m_ccaThreshDb; }

Ptr<LoraChannel> LoraPhyGen::GetChannel(void) const { return m_channel; }

Ptr<LoraNetDevice> LoraPhyGen::GetDevice(void) { return m_device; }

Ptr<LoraTransducer> LoraPhyGen::GetTransducer(void) { return m_transducer; }
void LoraPhyGen::SetChannel(Ptr<LoraChannel> channel) { m_channel = channel; }

void LoraPhyGen::SetDevice(Ptr<LoraNetDevice> device) { m_device = device; }

void LoraPhyGen::SetMac(Ptr<LoraMac> mac) { m_mac = mac; }

void LoraPhyGen::SetTransducer(Ptr<LoraTransducer> trans) {
  m_transducer = trans;
  m_transducer->AddPhy(this);
}

void LoraPhyGen::SetSleepMode(bool sleep) {
  if (sleep) {
    m_state = SLEEP;
    if (!m_energyCallback.IsNull()) {
...
void LoraPhyGen::SetUseRandomNoise(bool useNoise) {
  m_useRandomNoise = useNoise;
}

int64_t LoraPhyGen::AssignStreams(int64_t stream) {
  NS_LOG_FUNCTION(this << stream);
  m_pg->SetStream(stream);
  return 1;
}

void LoraPhyGen::NotifyTransStartTx(Ptr<Packet> packet, double txPowerDb,
                                    LoraTxMode txMode) {
  if (m_pktRx) {
    m_minRxSinrDb = -1e30;
  }
...
void LoraPhyGen::NotifyIntChange(void) {
...
double LoraPhyGen::CalculateSinrDb(Ptr<Packet> pkt, Time arrTime,
                                   double rxPowerDb, LoraTxMode mode,
                                   LoraPdp pdp) {
  double noiseDb =
      m_channel->GetNoiseDbHz((double)mode.GetCenterFreqHz() / 1000.0) +
      10 * std::log10(mode.GetBandwidthHz());
  return m_sinr->CalcSinrDb(pkt, arrTime, rxPowerDb, noiseDb, mode, pdp,
                            m_transducer->GetArrivalList());
}

double LoraPhyGen::GetInterferenceDb(Ptr<Packet> pkt) {
...
double LoraPhyGen::DbToKp(double db) { return std::pow(10, db / 10.0); }
double LoraPhyGen::KpToDb(double kp) { return 10 * std::log10(kp); }

void LoraPhyGen::NotifyListenersRxStart(void) {
  ListenerList::const_iterator it = m_listeners.begin();
  for (; it != m_listeners.end(); it++) {
    (*it)->NotifyRxStart();
  }
}
void LoraPhyGen::NotifyListenersRxGood(void) {
  ListenerList::const_iterator it = m_listeners.begin();
  for (; it != m_listeners.end(); it++) {
    (*it)->NotifyRxEndOk();
  }
}
void LoraPhyGen::NotifyListenersRxBad(void) {
  ListenerList::const_iterator it = m_listeners.begin();
  for (; it != m_listeners.end(); it++) {
    (*it)->NotifyRxEndError();
  }
}
void LoraPhyGen::NotifyListenersCcaStart(void) {
  ListenerList::const_iterator it = m_listeners.begin();
  for (; it != m_listeners.end(); it++) {
    (*it)->NotifyCcaStart();
  }
}
void LoraPhyGen::NotifyListenersCcaEnd(void) {
  ListenerList::const_iterator it = m_listeners.begin();
  for (; it != m_listeners.end(); it++) {
    (*it)->NotifyCcaEnd();
  }
...
void LoraPhyGen::NotifyListenersTxStart(Time duration) {
  ListenerList::const_iterator it = m_listeners.begin();
  for (; it != m_listeners.end(); it++) {
...
uint32_t LoraPhyGen::GetNModes(void) { return m_modes.GetNModes(); }

LoraTxMode LoraPhyGen::GetMode(uint32_t n) {
  NS_ASSERT(n < m_modes.GetNModes());

  return m_modes[n];
}

Ptr<Packet> LoraPhyGen::GetPacketRx(void) const { return m_pktRx; }
