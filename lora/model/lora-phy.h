...
/**
 *
 * Class used for calculating SINR of packet in LoraPhy.
 *
 */
class LoraPhyCalcSinr : public Object {
public:
  /**
  ...
  CalcSinrDb(Ptr<Packet> pkt, Time arrTime, double rxPowerDb, double ambNoiseDb,
             ...
             const LoraTransducer::ArrivalList &arrivalList) const = 0;
  /**
   * Register this type.
 ...
 * and modulation (mode).
 *
 * Can be set in LoraPhy via attributes.
 */
class LoraPhyPer : public Object {
public:
  /**
   ...
   */
  virtual double CalcPer(Ptr<Packet> pkt, double sinrDb, LoraTxMode mode) = 0;
...
public:
  ...
  /** Called when LoraPhy begins receiving packet. */
  virtual void NotifyRxStart(void) = 0;
  /** Called when LoraPhy finishes receiving packet without error. */
  virtual void NotifyRxEndOk(void) = 0;
  /** Called when LoraPhy finishes receiving packet in error. */
  virtual void NotifyRxEndError(void) = 0;
  /** Called when LoraPhy begins sensing channel is busy. */
  virtual void NotifyCcaStart(void) = 0;
  /** Called when LoraPhy stops sensing channel is busy. */
...
/**
 *
 * Base class for LORA Phy models.
 */
...
public:
   ...
   */
  typedef Callback<void, Ptr<Packet>, double, LoraTxMode> RxOkCallback;
  ...
  /**
   * TracedCallback signature for LoraPhy packet send/receive events.
   *
   * \param [in] pkt The packet.
   * \param [in] sinr The SINR.
   * \param [in] mode The channel mode.
   */
  typedef void (*TracedCallback)(const Ptr<const Packet> pkt, const double sinr,
                                 const LoraTxMode mode);

  /**
   * Set the DeviceEnergyModel callback for LoraPhy device.
   *
   ...
   * \param modeNum  Index of mode in SupportedModes list to use for
   * transmission.
   */
  virtual void SendPacket(Ptr<Packet> pkt, uint32_t modeNum) = 0;

  /**
   * Register a LoraPhyListener to be notified of common LoraPhy events.
   *
   * \param listener New listener to register.
   */
  virtual void RegisterListener(LoraPhyListener *listener) = 0;
  ...
  virtual void StartRxPacket(Ptr<Packet> pkt, double rxPowerDb,
                             LoraTxMode txMode, LoraPdp pdp) = 0;
   ...
   */
  virtual Ptr<LoraNetDevice> GetDevice(void) = 0;
   ...
   */
  virtual void SetDevice(Ptr<LoraNetDevice> device) = 0;
   ...
   * \param mac The MAC.
   */
  virtual void SetMac(Ptr<LoraMac> mac) = 0;
   ...
   */
  virtual void SetTransducer(Ptr<LoraTransducer> trans) = 0;
   ...
   */
  virtual Ptr<LoraTransducer> GetTransducer(void) = 0;
   ...
   * \return The mode.
   */
  virtual LoraTxMode GetMode(uint32_t n) = 0;
  ...
  /**
   * Turn off/on random noise in receive.
   * \param noiseOn true turns on false off
   */
  virtual void SetUseRandomNoise(bool noiseOn) = 0;
